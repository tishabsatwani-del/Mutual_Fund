...
node  tools/tool-tests/upload.test.js        # every way a file gets in, and every refusal
...
**`upload.test.js`** drives the three file doors with files in the shapes real
ones arrive in, generated by `fixtures/make_upload_fixtures.py`: NSE's index
export with a title above thirteen columns of which only one is the index
value, AMFI's semicolons, a fund house's `.xlsx` whose header is on row four, a
Zerodha tradebook, a genuine PDF, and the same PDF renamed `.csv`. It exists
because three of those were being refused, two of them with the words
"contains trade logs or transaction records" — a diagnosis that was both wrong
and unactionable. It also holds the promise the upload control makes: that the
box the reader clicked is the box that answers, checked by measuring the
confirmation's position against the viewport rather than by trusting the
markup.
